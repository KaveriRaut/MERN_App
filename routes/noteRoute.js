...
//getting the newNote data from frontend url 'http://localhost:3001/create'
//accessing '/create' route and create newNote at backend by POST method and save to mongoDB
router.route("/create").post((req,res) => {
    const title = req.body.title;
    const content = req.body.content;
    const newNote = new Note({ //'new Note' model is created for newNote at backend
        title,
        content
    });
    newNote.save();
...
// # To Display Details of Single Notes
...
router.route("/update/:id").put( async (req, res) => {
    ...
    try{
        ...
        const updateNote = await Note.findByIdAndUpdate( req.params.id, { $set: req.body });
